...
export function Alert({
  ...
  return (
    <div 
      className={`border-l-4 p-4 rounded-r-md ${variants[variant]} ${className}`}
      role="alert"
      {...props}
    >
      <div className="flex items-start">
        ...
        {dismissible && onDismiss && (
          <button
            type="button"
            className="ml-auto -mx-1.5 -my-1.5 rounded-full p-1.5 inline-flex text-current hover:bg-opacity-10 hover:bg-current focus:outline-none"
            onClick={onDismiss}
            aria-label="Dismiss"
          >
